...
! PG off centering:
  ...
  integer, pointer :: layout(:), io_layout(:)
   ...
   real    :: too_big  = 1.E35
#endif
   public :: fv_init1, fv_init2, fv_end
...
!! sets up the grid, and controls FV3 namelist parameters.   
 subroutine fv_init1(Atm, dt_atmos, grids_on_this_pe, p_split)
...
!     if ( beta < 1.e-5 ) beta = 0.   ! beta < 0 is used for non-hydrostatic "one_grad_p"
      ...
      if ( .not.hydrostatic ) then
         if ( m_split==0 ) then
            m_split = CEILING(1. + abs(dt_atmos)/real(k_split*n_split*abs(p_split)))
